  ...
  export default {
    ...
    methods: {
      ...
      handleInputEditConfirm(item,tag,index) {
        console.log("aaaaaaTag",tag)
        if(this.inputValue)
        {
          const inputValue = this.inputValue
          console.log("aaaaaaTag",tag)
          item.args[tag]=inputValue
        }
        this.currentTagIndex=-1
      ...
      //提交路由
      handleSubmit() {
        let { predicates, filters, ...other } = JSON.parse(JSON.stringify(this.router))
        let router = other
        router.predicates = JSON.stringify(this.router.predicates)
        router.filters = JSON.stringify(this.router.filters)
        console.log("当前的Routes",this.router)
        if(!this.router.hasOwnProperty("id")){
          //将athis.router.predicates
          let routerJson = JSON.parse(JSON.stringify(this.router.predicates))
          
          for(let predicate in routerJson){
            console.log(predicate)
            let obj = {}
            let index = 0;
            for(let key in routerJson[predicate].args){
              
              obj[key] = routerJson[predicate].args[index]
              index++
            }
            routerJson[predicate].args = obj
          }
          this.router.predicates=routerJson
          this.routes.push(this.router)
        }
